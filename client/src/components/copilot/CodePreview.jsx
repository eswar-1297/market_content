...
function stripSourceBlocks(html) {
  ...
  cleaned = cleaned.replace(/<p[^>]*><em>[^<]*(?:Source|Sourced|FAQ pipeline|Fanout|Google PAA|Reddit|Quora|Internal SharePoint|Internal Migration Docs|verify link|verify product)[^<]*<\/em><\/p>/gi, '')
  ...
  cleaned = cleaned.replace(/<p[^>]*>[^<]*(?:🔗\s*Internal SharePoint|🔗\s*Internal Migration Docs|⚠️\s*verify)[^<]*<\/p>/gi, '')
  cleaned = cleaned.replace(/<li[^>]*>[^<]*(?:🔗\s*Internal SharePoint|🔗\s*Internal Migration Docs|⚠️\s*verify link|📋\s*Sources)[^<]*<\/li>/gi, '')
